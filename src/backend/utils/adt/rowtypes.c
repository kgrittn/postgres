...
/*
 * record_id :
 *		  compares two records for identical contents
 * result :
 *		  returns true if the records are identical, false otherwise.
 *
 * Note: we do not use record_cmp here, since identity may be meaningful in
 * datatypes that don't have a total ordering (and hence no btree support).
 */
Datum
record_id(PG_FUNCTION_ARGS)
{
	HeapTupleHeader record1 = PG_GETARG_HEAPTUPLEHEADER(0);
	HeapTupleHeader record2 = PG_GETARG_HEAPTUPLEHEADER(1);
	bool		result = true;
...
Datum
record_ni(PG_FUNCTION_ARGS)
{
	PG_RETURN_BOOL(!DatumGetBool(record_id(fcinfo)));
}
